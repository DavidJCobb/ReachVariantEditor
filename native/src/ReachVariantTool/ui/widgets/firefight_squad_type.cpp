...
FFSquadTypeWidget::FFSquadTypeWidget(QWidget* parent) : QComboBox(parent) {
   {
      ...
      _append(this, reach::firefight_squad::heretic_snipers,
         tr("Heretic Snipers", disambig),
         tr("Squads of Elites wielding human sniper rifles.", disambig)
      );
      _append(this, reach::firefight_squad::heretic_heavy,
         tr("Heretic Heavy", disambig),
         tr("Squads of Elites wielding human explosive weaponry.", disambig)
      );
   }
   this->model()->sort(0); // TODO: can we put "None" at the top of the list?
   this->setCurrentIndex(this->findData(QVariant((int)reach::firefight_squad::brutes), Qt::ItemDataRole::UserRole));
   //
   QObject::connect(this, QOverload<int>::of(&QComboBox::currentIndexChanged), [this](int index) {
      if (!this->target)
         return;
      auto value = this->currentData(Qt::ItemDataRole::UserRole).toInt();
      *this->target = (reach::firefight_squad)value;
   });
}
...
void FFSquadTypeWidget::_updateFromTarget() {
   ...
   this->setDisabled(this->target == nullptr);
   //
   auto to_select = reach::firefight_squad::brutes;
   if (this->target)
      to_select = *this->target;
   this->setCurrentIndex(this->findData(QVariant((int)to_select), Qt::ItemDataRole::UserRole));
}