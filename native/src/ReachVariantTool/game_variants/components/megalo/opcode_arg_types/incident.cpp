...
namespace Megalo {
   ...
   arg_compile_result OpcodeArgValueIncident::compile(Compiler& compiler, cobb::string_scanner& arg, uint8_t part) noexcept {
      if (part > 0)
      ...
      int32_t value = 0;
      QString word;
      auto    result = compiler.try_get_integer_or_word(arg, value, word, QString("incident names"), nullptr, max_value, &OpcodeArgValueIncident::typeinfo);
