...
namespace Megalo {
   ...
   /*virtual*/ bool OpcodeArgValueObject::read(cobb::ibitreader& stream) noexcept /*override*/ {
      ...
      const VariableScope* biped_var_scope = nullptr;
      switch ((_scopes)this->scope) {
         ...
         case _scopes::player_player_biped:
            biped_var_scope = &MegaloVariableScopePlayer;
            break;
         case _scopes::object_player_biped:
            biped_var_scope = &MegaloVariableScopeObject;
            break;
         case _scopes::team_player_biped:
            biped_var_scope = &MegaloVariableScopeTeam;
            break;
         default:
            {
      ...
      if (variable_scope) {
         ...
         return true;
      } else if (biped_var_scope) {
         this->which = stream.read_bits<uint16_t>(biped_var_scope->which_bits());
         this->index = stream.read_bits<uint16_t>(biped_var_scope->index_bits(variable_type::player));
         if (!biped_var_scope->is_valid_which(this->which)) {
            auto& error = GameEngineVariantLoadError::get();
            error.state    = GameEngineVariantLoadError::load_state::failure;
            error.reason   = GameEngineVariantLoadError::load_failure_reason::bad_script_opcode_argument;
            error.detail   = GameEngineVariantLoadError::load_failure_detail::bad_variable_scope;
            error.extra[0] = (int32_t)getScopeConstantForObject(*biped_var_scope);
            error.extra[1] = this->which;
            error.extra[2] = this->index;
   ...
   void OpcodeArgValueObject::write(cobb::bitwriter& stream) const noexcept {
      ...
      if (variable_scope) {
         ...
         stream.write(this->which, biped_var_scope->which_bits());
         stream.write(this->index, biped_var_scope->index_bits(variable_type::player));
         return;
      }
      if (which_bits)
